...
export default function SignUp() {
  ...
  // Generate random username: user_ + random 6 digits
  ...
  const handleSignup = async () => {
    ...
    if (!mobile.trim()) return Alert.alert("Required", "Please enter your mobile number");
    if (mobile.length < 10) return Alert.alert("Invalid", "Mobile number must be at least 10 digits");
    ...
    try {
        ...
        const randUsername = generateUsername();
        const [first, ...last] = name.trim().split(" ");

        formData.append("email", email.trim().toLowerCase());
        formData.append("username", randUsername);
        formData.append("password", password.trim());
        formData.append("mobile", mobile.trim());
        formData.append("first_name", first || "");
        formData.append("last_name", last.join(" ") || "");
        
        // Default/Empty values for optional fields to satisfy potential backend checks
